...
- **Authorization:** Three roles — Customer (session-scoped access to their own Orders/Addresses/Order History only), Admin (a Better Auth `role` on the `user` table, via the `admin` plugin; gated access to `/admin` — users list with role management, and an orders dashboard that can ship a confirmed order), and Owner (Sanity Studio access, managed by Sanity's own project-member auth, not Better Auth, for catalog/CMS content). `POST /api/orders/[id]/ship` accepts either an authenticated admin session or the legacy `x-admin-secret` header used by the Sanity Studio "Ship Order" tool.
